...
metrics={'federated_accuracy':round(rows[0]['accuracy'],3),'dp_accuracy_noise_0_10':round(rows[3]['accuracy'],3),'utility_delta':round(rows[0]['accuracy']-rows[3]['accuracy'],3)}; (root/'results'/'demo_metrics.json').write_text(json.dumps(metrics,indent=2)); print(json.dumps(metrics,indent=2))
